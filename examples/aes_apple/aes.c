...
void Harness(Byte *Ciphertext, const Byte *Plaintext,
	const AESData *ExpandedKey, long Nr) {

	public_in(__SMACK_value(Ciphertext));
	public_in(__SMACK_value(Plaintext));
	public_in(__SMACK_value(Nr));
	AESEncryptWithExpandedKey(Ciphertext, Plaintext, ExpandedKey, Nr);
}
